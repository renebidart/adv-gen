...
class PreActBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.topk_layer1 = SpatialTopK(topk=1, frac=.05, groups=1)
        self.topk_layer2 = SpatialTopK(topk=1, frac=.05, groups=1)

    def forward(self, x):
        out = self.topk_layer1(self.bn1(x))
        shortcut = self.shortcut(out) if hasattr(self, 'shortcut') else x
        out = self.conv1(out)
        out = self.conv2(self.topk_layer2(self.bn2(out)))
        out += shortcut
        return out
...
class PreActBottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.topk_layer1 = SpatialTopK(topk=1, frac=.05, groups=1)
        self.topk_layer2 = SpatialTopK(topk=1, frac=.05, groups=1)
        self.topk_layer3 = SpatialTopK(topk=1, frac=.05, groups=1)

    def forward(self, x):
        out = self.topk_layer1(self.bn1(x))
        shortcut = self.shortcut(out) if hasattr(self, 'shortcut') else x
        out = self.conv1(out)
        out = self.conv2(self.topk_layer1(self.bn2(out)))
        out = self.conv3(self.topk_layer1(self.bn3(out)))
        out += shortcut
        return out


class PResNetRegNoRelU(nn.Module):
    def __init__(self, depth, frac=None, groups=1, num_classes=10):
        super(PResNetRegNoRelU, self).__init__()
        self.in_planes = 64
